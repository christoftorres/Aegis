...
def request_debug_trace(connection, transaction_hash, disable_stack=False, disable_memory=False, disable_storage=True, retries=0):
        ...
        try:
            ...
            if str(e) == "Remote end closed connection without response":
                try:
                    ...
                    pass
            if tries < 10:
                delay = random.randint(60, 600) # Wait between 1 and 10 min
                print("Retrying retrival of trace "+str(transaction_hash)+" in "+str(int(delay/60))+" min.")
                time.sleep(delay)
            else:
                return {"error": e}
